class File_Processor
  ...
  def current_time
    time = DateTime.now.strftime('%m-%d-%Y %H:%M:%S:%L %Z')
  end
  ...
  def create_file
    if File.exist?('Cats.txt')
      ...
      birth_time = File.birthtime('Cats.txt').strftime('%m-%d-%Y %H:%M:%S:%L %Z')
  ...
  def modify_file
    ...
    mod_time = File.mtime('Cats.txt').strftime('%m-%d-%Y %H:%M:%S:%L %Z')
